  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  cfg_.range_name = info_.hardware_parameters["range_name"];
  ...
  for(const hardware_interface::ComponentInfo & sensor : info_.sensors)
  {
    ...
    if(sensor.name.c_str() == cfg_.range_name)
    {
...
std::vector<hardware_interface::StateInterface> BotHardwareSystem::export_state_interfaces()
{
  ...
  state_interfaces.emplace_back(hardware_interface::StateInterface(
    range_.name, "range", &range_.range));
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & period)
{
  ...
  comms_.read_range_sensor(range_.range);
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  //   if(wheel_r_.cmd > wheel_l_.cmd && wheel_l_.cmd > 0) left_desired_speed = 0.0;
  ...
  //     right_desired_speed = 30;
  //   } 
  //   if(wheel_r_.cmd < wheel_l_.cmd){
  //     left_desired_speed = 30;
  ...
  // comms_.set_motor_values(1, 1); 
  // comms_.set_motor_values(motor_r_counts_per_loop, motor_l_counts_per_loop); //inversed pins

  // int motor_l_counts_per_loop = left_desired_speed *10;
  // int motor_r_counts_per_loop = right_desired_speed *10;
  // comms_.run_motor_pwm(motor_l_counts_per_loop, motor_r_counts_per_loop);
  ...
  // if (wheel_l_.vel != 0 || wheel_r_.vel != 0){
  //   RCLCPP_INFO(rclcpp::get_logger("BotHardwareSystem"), "Left wheel: %d Right wheel: %d", motor_l_counts_per_loop, motor_r_counts_per_loop);
  //   RCLCPP_INFO(rclcpp::get_logger("BotHardwareSystem"), "true L wheel: %f, true R wheel: %f", wheel_l_.vel, wheel_r_.vel);
  //   RCLCPP_INFO(rclcpp::get_logger("BotHardwareSystem"), "Left pos: %f, right pos: %f", wheel_l_.pos, wheel_r_.pos);
  // }
