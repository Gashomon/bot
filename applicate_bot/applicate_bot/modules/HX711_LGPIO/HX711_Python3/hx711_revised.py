...
class HX711:
    ...
    def __init__(self, device,
        ...
        lgpio.gpio_claim_output(self.device, self._pd_sck)  # pin _pd_sck is output only
        lgpio.gpio_claim_input(self.device, self._dout)  # pin _dout is input only
    ...
    def _ready(self):
        ...
        if lgpio.gpio_read(self.device, self._dout) == 0:
            return True
        else:
            return False
    ...
    def _set_channel_gain(self, num):
        ...
        for _ in range(num):
            ...
            lgpio.gpio_write(self.device, self._pd_sck, 1)
            lgpio.gpio_write(self.device, self._pd_sck, 0)
    ...
    def _read(self):
        ...
        """
        lgpio.gpio_write(self.device, self._pd_sck, 0)  # start by setting the pd_sck to 0
        ready_counter = 0
        ...
        for _ in range(24):
            ...
            data_in = (data_in << 1) | lgpio.gpio_read(self.device, self._dout)
