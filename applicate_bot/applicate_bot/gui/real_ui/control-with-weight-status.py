...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.simulate_weight_status()

        self.timer = QtCore.QTimer()
        self.timer.timeout.connect(self.simulate_weight_status)  
    ...
    def simulate_weight_status(self):
        """ Simulate and display the weight status """
        self.simulated_weight = random.randint(1000, 20000)  
        self.weight_limit = 15000  

        self.update_weight_status()

    def update_weight_status(self):
        """ Update weight status and display it in the window """
        if self.simulated_weight > self.weight_limit:
            self.weight_status_label.setText(f"Weight: {self.simulated_weight / 1000:.2f} kg - Overloaded!")
            self.weight_status_label.setStyleSheet("color: red;")
            self.show_overload_warning()
        else:
            self.weight_status_label.setText(f"Weight: {self.simulated_weight / 1000:.2f} kg")
            self.weight_status_label.setStyleSheet("color: green;")

    def show_overload_warning(self):
        """ Show a pop-up warning when the weight exceeds the limit """
        warning_message = QtWidgets.QMessageBox()
        warning_message.setIcon(QtWidgets.QMessageBox.Warning)
        warning_message.setWindowTitle("Weight Overload")
        warning_message.setText("The weight has exceeded the 15 kg limit!, Reduce some weight.")
        warning_message.setStandardButtons(QtWidgets.QMessageBox.Ok)
       
        warning_message.setStyleSheet("QLabel { font-size: 15px; }")  
        warning_message.exec()

        
        self.pushButton_2.clicked.connect(self.show_sender_receiver)
        self.pushButton_3.clicked.connect(self.show_sender_receiver)
        self.pushButton_4.clicked.connect(self.show_sender_receiver)

    def show_sender_receiver(self):
        """ Show sender and receiver name text boxes when any button is clicked """
